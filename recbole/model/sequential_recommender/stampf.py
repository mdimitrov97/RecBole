...
class STAMPF(SequentialRecommender):
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        input_concat = torch.cat((item_seq_emb, feature_emb), -1)  # [B 1+field_num*H]
        input_emb = self.concat_layer(input_concat)
        last_inputs = self.gather_indexes(input_emb, item_seq_len - 1)

        org_memory = input_emb
